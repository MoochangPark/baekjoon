...
---
### 23년 9월 16일
- [x] 2467 - 투 포인터 문제. 어느게 0에 가까운지는 절대값을 씌우면 해결
- [x] 11000 - 그리디 문제. 시작 시간 기준 오름차순 된 리스트에서 첫번째를 시작으로 다음 시간대의 시작시간이 현재 가장 짧게 남은 시간의 끝시간보다 작으면 새 방이 있어야한다. 만약 더 크다면 해당 시간이 끝나고 해당 방을 이어서 쓰면 된다.
---
### 23년 9월 17일
- [x] 1915 - 계산 문제. 정사각형은 대각선, 위, 아래의 dp값을 이용하면 된다.
---
### 23년 9월 18일
- [x] 2638 - bfs 문제. 치즈들이 대각선으로 연결되어 있을 수 있어 내부 탐색은 불가. 따라서 외부에서 공기인지 아닌지로 치즈의 테두리 확인.
---
### 23년 9월 19일
- [x] 5052 - 정렬 문제. 정렬을 하면 앞에서부터 같은 숫자들 순으로 정렬되기 떄문에 이후로는 하나씩 비교하면 된다.
- [x] 15684 - dfs 문제. 내 위치에서 오른쪽 아래 끝까지 모든 dfs를 그려봐야하는데, 내가 있는 세로선이면 내 현재 깊이를 고려해야하고, 아니면 0깊이부터 그린다. 그리고 다음 선은 연결되게 그려지지 않도록 +2로 두줄 떨어진 곳에서 탐색한다.
---
### 23년 9월 20일
- [x] 1024 - 수학 문제. L부터 100까지 길이를 만들 수 있으면서, x+1부터 x+n 까지의 합이 N임으로 점화식을 찾아 i길이 만큼 출력하면된다.
---
### 23년 9월 21일
- [x] 17143 - 구현 문제. 판을 새로 만들지 말고, 기존의 판을 3차원배열로 만들어서 누적시키고 나중에 처리하는게 더 좋다. 그리고 이동 횟수가 너무 많기 때문에 mod를 사용해서 제자리로 돌아온 횟수 만큼을 뺴고 하면 시간이 짧아진다.
---
### 23년 9월 22일
- [x] 2357 - 세그먼트 트리 문제. 구간 범위에서 최소값이나 최대값 등 연산이 아닌 빠르게 찾아야 할 떄는 세그먼트 트리를 이용하는 것도 방법
---
### 23년 9월 23일
- [x] 1647 - MST 문제. 최소스패닝 트리로 각 마을 생성의 최소 비용을 구한 후에 마지막으로 두 마을을 이을 때 쓴 경로는 필요없으므로 뺴준다.
---
### 23년 9월 24일
- [x] 2749 - 분할정복 문제. 피사노 공식을 알아야 풀 수 있다. 결국 일반적인 피보나치지만, n값에 대해서 n % (15 * 100000)를 해줘야 한다.
---
### 23년 9월 25일
- [x] 16235 - 구현 문제. 봄 여름 가을 겨울을 잘 만들면 된다.
---
### 23년 9월 26일
- [x] 1600 - visit 배열 없이 큐에 계속 넣으면 메모리 초과가 발생한다. 차라리 범위가 괜찮으면 3차원 배열로 만들어서 처리하는게 나을 수 있다. 그리고 이러한 문제들은 대부분 먼저온 놈이 제일 빠른 것이므로 해당 배열에 초기값 상태인지 아닌지로 visit배열을 대체할 수 있다.
---
### 23년 9월 27일
- [x] 11657 - 밸만 포드 문제. 음수 가중치가 나오면 무조건 밸만포드일 가능성이 높다.
---
### 23년 9월 28일
- [x] 1016 - 에라토스테네스 문제. 기본적인 에라토스테네스에서 제곱 수 뿐만 아니라 세제곱, 네제곱을 다 없애 주어야 하며, 최초값을 int(min/t)를 통해 범위 이하의 수는 계산하지 않도록 해준다.
- [x] 7579 - 냅색 문제. 코스트가 기준이기 때문에 cost의 최대 합 만큼 dp를 돌려야하며, dp에 기록하는 건 cost가 아니라 weight다.
- [x] 2891 - 수학 문제. 먼저 최대공약수에 대한 점화식을 찾는다. 이후에는 최대공약수를 구하고, 최대공약수의 약수들을 찾으면 된다.
- [x] 1062 - dfs 문제. 26까지 밖에 안되므로 dfs가 가능. anta와 tica는 꼭 필요하므로 총 5개의 글자를 제외한 수만큼만 배울 수 있다. dfs를 통해 배울 수 있는 경우의 수의 개수가 충족되면, 각 문자열 마다 배운 글자로 읽을 수 있는 단어인지 확인하면 된다.
---
### 23년 9월 29일
- [x] 10986 - 누적합 문제. 기본 누적합에서 나머지한 결과들을 가지고 수학 식을 찾아야 함.
---
### 23년 9월 30일
- [x] 4195 - 유니온 파인드 문제. 유니온 단계에서 연결시킴과 동시에 누적 계산도 가능
---
### 23년 10월 1일
- [x] 1076 - 구현 문제. 리스트의 인덱스로 계산하면 수월하다.
---
### 23년 10월 2일
- [x] 2263 - 트리 문제. 5639와 비슷한 문제. 
---
### 23년 10월 3일
- [x] 11779 - 다익스트라문제. 경로는 이전 경로들을 적고 도착지에서 출발지로 회귀하며 찾는다.
---
### 23년 10월 4일
- [x] 2011 - dp 문제. 1글자와 2글자로 만들수 있는 경우에 대해 확인. 현재 위치에서 만들 수 있는 1자리와 2자리의 경우의 수들이 다음 위치에 반영되는 것을 기준으로 한다.
---
### 23년 10월 5일
- [x] 11758 - 수학 문제. 3개의 점을 이었을 떄 방향이 왼쪽으로 가면 각이 작아지는거고, 방향이 오른쪽으로 가면 각이 커지는 것.
---
### 23년 10월 6일
- [x] 1516 - 위상정렬 문제. 핵심은 우선 진입차수가 0인 건물은 자신의 시간만큼 걸리고, 나머지 건물들은 해당 건물을 짓기 위한 우선 조건의 건물을 짓는데 걸리는 시간에 해당 건물을 짓는데 걸리는 시간을 더해서 올린다. 이때 동시에 여러 건물을 올릴 수 있어 우선 조건의 건물들 중 가장 오래걸리는 것을 올린다.
---
### 23년 10월 7일
- [x] 17135 - 구현 문제. 조건을 잘 확인해야하며, 하나씩만 찾기 보다는 그냥 쏠 수 있는 적들을 전부 받고 하나를 뽑아내는 것이 안전.
---
### 23년 10월 8일
- [x] 1100 - 구현 문제. 조건을 잘 맞추면 됨.
---
### 23년 10월 9일
- [x] 1145 - 브루트포스 문제. 어느 범위를 선정할 수 없으니 while로 처리해야함.
---
### 23년 10월 10일
- [x] 1159 - 문자열 문제. ord와 chr을 적절하게 사용하는것이 중요.
---
### 23년 10월 11일
- [x] 1173 - 구현 문제. 처음부터 m+T가 M을 넘어버리면 휴식부터 해야하는데, 그럼 초기보다 낮아지면서 계속 초기 맥박 상태로만 되는 무한루프가 반복됨.
---
### 23년 10월 12일
- [x] 4179 - bfs 문제. 불보다 주인공이 먼저 움직여야 불에 안 죽을 수 있으므로, 초기 q에 입력할 때 주인공 위치 먼저 들어갈수 있도록 해야함.
---
### 23년 10월 13일
- [x] 14719 - 구현 문제. 현재 칸을 중심으로 가장 높은 것을 찾아야 한다. 비록 높은거 뒤에 더 높은 것을 찾을 수 있지만, 결국 왼쪽과 오른쪽 중에 낮은 것을 기준으로 채울 수 있는 물의 양을 계산하기 때문에 max로 해도 됨.
---
### 23년 10월 14일
- [x] 2166 - 수학 문제. 리스트를 만들 때 리스트의 첫번째의 값을 한 번 더 넣어주어야 한다.
---
### 23년 10월 15일
- [x] 2239 - 구현 문제. 조건 a,b,c가 다 만족해야한다는 것은 이중 하나라도 false일때 바로 다음으로 넘어가도록 가지치기를 해야한다.
---
### 23년 10월 16일
- [x] 1212 - int함수는 input 뒤에 콤마로 구분 후 숫자를 적으면 해당 진수로 입력받는다.
---
### 23년 10월 17일
- [x] 2437 - 그리디 문제. 1,2,3이 있을때 1은 1, 2는 2, 3은 3, 4는 1+2, 5는 2+3, 6은 1+2+3으로 만들 수 있다. 즉, 기준값보다 낮은 숫자들은 어떻게 더하든 전부 다 만들 수 있다. 따라서 기준값 보다 큰 값이 올 경우만 막아주면 된다.
---
### 23년 10월 18일
- [x] 4386 - MST 문제. cost를 직접 계산하고, a와 b를 cost계산할때의 i와 j값으로 치환하면 어렵지 않음.
---
### 23년 10월 19일
- [x] 1225 - 구현 문제. 범위가 적어 전부 하나씩 꺼내서 int형으로 변환 후 사용
---
### 23년 10월 20일
- [x] 2623 - 위상 정렬 문제. 리스트에서 i+1번째는 i번째 이후에 와야한다는 것을 찾아내 graph에 넣을 수 있으면 됨.
---
### 23년 10월 21일
- [x] 17142 - bfs문제. 비활성 바이러스를 활성으로 바꿀 경우와 안바꿀 경우를 모두 생각해야함.
---
### 23년 10월 22일
- [x] 5427 - bfs 문제. 4179와 비슷하나 불이 붙을 곳도 안되기 떄문에 처음에 불을 먼저 번지도록 해야함.
---
### 23년 10월 23일
- [x] 1267 - 구현문제. 범위를 고려하면 굳이 if와 else로 나눌 필요가 없다.
---
### 23년 10월 24일
- [x] 1259 - 펠린드롬 문제. 길이가 길지 않아 양 끝에서 확인하면 가능.
---
### 23년 10월 25일
- [x] 1058 - 플로이드 와셜 문제. 직접 연결되거나 중간을 걸쳐서 만날경우 친구임을 추가한 후 나중에 행 단위로 그 수를 조사하면 됨.
---
### 23년 10월 26일
- [x] 1251 - 구현 문제. reverse와 join을잘활용해보면 됨.
---
### 23년 10월 27일
- [x] 1254 - 부르트포스 문제. 왼쪽에서 얼마만큼을 잘라서 붙여야 펠린드롬이 되는지 확인하는게 핵심.
---
### 23년 10월 28일
- [x] 1270 - 구현 문제. 값을 추가할때마다 비교하며 땅을 점령했는지 판별하는 것이 중요.
---
### 23년 10월 29일
- [x] 1292 - 누적합 문제. 구간에 따라 0부터 시작해서 넣고 구간을 구함
---
### 23년 10월 30일
- [x] 1236 - 구현 문제. 행과 열 모두 있어야하므로 max값을 찾는게 중요.
---
### 23년 10월 31일
- [x] 1247 - 구현 문제. 파이썬은 타입지정이 필요없다.
---
### 23년 11월 1일
- [x] 2576 - 구현 문제. 7개의 조건을 확인.
---
### 23년 11월 2일
- [x] 27433 - 구현 문제. 0일때 1인것을 고려.
---
### 23년 11월 3일
- [x] 20492 - 계산 문제. math.ceil을 이용해 소수점 삭제.
---
### 23년 11월 4일
- [x] 15963 - 수학 문제. 화이팅.
---
### 23년 11월 5일
- [x] 1919 - 구현문제. counter을 사용하면 편리.
---
### 23년 11월 6일
- [x] 2965 - 수학 문제. 범위 안에서 놀게 되므로, 횟수 또한 동일하다.
---
### 23년 11월 7일
- [x] 9085 - 수학 문제. 화이팅.
